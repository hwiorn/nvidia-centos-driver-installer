...
configure_nvidia_installation_dirs() {
  ...
  # filesystem.
  mkdir -p bin bin-workdir /usr/tbin
  export PATH=$PATH:/usr/tbin
  mount -t overlay -o lowerdir=/usr/bin,upperdir=bin,workdir=bin-workdir none /usr/tbin
  ...
  trap "{ umount /lib/modules/${KERNEL_VERSION}/video; umount /usr/lib/x86_64-linux-gnu ; umount -l /usr/tbin; }" EXIT
  popd
